...
def test_lab_manifest_uses_embedded_visualisation_model():
    ...
    assert "boltz_boltz2_affinity_predictor" in model_aliases
...
def test_lab_manifest_is_guided_boltz_workflow():
    lab_dir = Path(__file__).resolve().parents[1]
    manifest = yaml.safe_load((lab_dir / "lab.yaml").read_text())

    assert manifest["title"] == "Boltz Workflow: Protein-Ligand 101"
    ...
    assert params["default_ligand_smiles"]
    assert params["default_run_options"]["workflow_name"] == "Protein-Ligand 101"
