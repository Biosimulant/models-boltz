...
EXPECTED_ALIASES = ["cancer_kinase_context", "abl1_imatinib_setup", "boltz_boltz2_affinity_predictor", "kinase_prediction_interpreter", "visualisation"]
...
def test_lab_manifest_uses_multi_stage_workflow_graph():
    ...
    assert "cancer_kinase_context.scenario_context" in wiring_sources
    assert "abl1_imatinib_setup.assembled_boltz_request" in wiring_sources
    assert "kinase_prediction_interpreter.prediction_evidence" in wiring_sources
...
def test_lab_manifest_keeps_boltz_prediction_as_only_scientific_runner():
    ...
    context_model = next(entry for entry in manifest["models"] if entry["alias"] == "cancer_kinase_context")
    assembler_model = next(entry for entry in manifest["models"] if entry["alias"] == "abl1_imatinib_setup")
    core_model = next(entry for entry in manifest["models"] if entry["alias"] == "boltz_boltz2_affinity_predictor")
    interpreter_model = next(entry for entry in manifest["models"] if entry["alias"] == "kinase_prediction_interpreter")
...
def test_public_ports_route_through_workflow_stages():
    ...
    assert all(item.startswith("abl1_imatinib_setup.") for item in input_maps)
    output_maps = [entry["maps_to"] for entry in manifest["io"]["outputs"]]
    assert "cancer_kinase_context.scenario_context" in output_maps
    assert "abl1_imatinib_setup.assembled_boltz_request" in output_maps
    assert "kinase_prediction_interpreter.prediction_evidence" in output_maps
