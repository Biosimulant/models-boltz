...
def test_visualisation_model_renders_docking_visuals(tmp_path):
    ...
    elif mode == "boltz_batch":
        structure_path = tmp_path / "complex.cif"
        structure_path.write_text("data_test\n#\n", encoding="utf-8")
        inputs = {
            f"{alias}_run_metadata": completed,
            f"{alias}_structure_artifacts": _record(f"{alias}_structure_artifacts", {"structure_file": str(structure_path)}),
            f"{alias}_confidence_summary": _record(f"{alias}_confidence_summary", {"confidence_score": 0.91}),
            f"{alias}_affinity_summary": _record(f"{alias}_affinity_summary", {"affinity_pred_value": -8.4, "affinity_probability_binary": 0.81}),
            f"{alias}_batch_summary": _record(
                f"{alias}_batch_summary",
                {
                    "ranked_ligands": [
                        {
                            "rank": 1,
                            "ligand": "Ligand A",
                            "binder_probability": 0.81,
                            "affinity_like_value": -8.4,
                            "confidence": 0.91,
                            "flags": ["review pose before follow-up"],
                        }
                    ]
                },
            ),
