# Working implementation specification, v1

Status: non-authoritative implementation record; formal MRS/MTS approval is not claimed.

Adapter fit: preserve upstream pretrained Boltz inference and compose explicit molecular/structural data boundaries. CandidateModel validates MSA and chemistry; PredictionModel invokes seven total native CLI calls across the three selected questions; AnalysisModel verifies raw scores and structures, contacts, rigid alignment and statistics. All modules execute once before run in dependency order. Ports are typed records with field-specific units. Dependencies are exactly pinned in model.yaml.

Acceptance remains PROTOCOL.md unchanged. Eight local scientific transformation tests passed before Lab submission; these are not inference results. Native and Lab paths must preserve inputs, Boltz version, seeds and sampling controls. Actual backend GPU, dependency freeze and checkpoint digests are collected at execution. Any parity mismatch remains a reportable result.
