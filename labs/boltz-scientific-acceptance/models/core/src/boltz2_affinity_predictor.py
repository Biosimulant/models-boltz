...
class Boltz2AffinityPredictor(BioModule):
    ...
    def __init__(
        ...
        if any(not isinstance(c, str) or not c or any(ch.isspace() for ch in c) for c in (protein_chain_id, ligand_chain_id)) or protein_chain_id == ligand_chain_id:
            raise ValueError("Protein and ligand chain IDs must be distinct nonempty identifiers")
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            ...
            "ligand_smiles": SignalSpec.scalar(
                contract={'profile': 'chemical.smiles/v1'},
                dtype="str",
                format="smiles",
                description="One molecule represented as molecular SMILES.",
            ),
            "msa_path": SignalSpec.scalar(
                contract={'profile': 'protein.multiple-sequence-alignment/v1'},
                dtype="str",
                value_type="file",
                format="a3m",
                required=False,
    ...
    def outputs(self) -> dict[str, SignalSpec]:
        return {
            "requested_chains": SignalSpec.record(schema={"protein": "str", "ligand": "str"}),
            "requested_protein_sequence": SignalSpec.scalar(dtype="str", format="sequence", contract={"profile": "protein.sequence/v1", "species": "any"}),
            "requested_ligand_smiles": SignalSpec.scalar(dtype="str", format="smiles", contract={"profile": "chemical.smiles/v1"}),
    ...
    def _execute_at_time(self, start: float, end: float) -> None:
        ...
        atomic_outputs: Dict[str, Any] = {
            "requested_chains": {"protein": self.protein_chain_id, "ligand": self.ligand_chain_id},
            "requested_protein_sequence": self._protein_sequence,
            "requested_ligand_smiles": self._ligand_smiles,
