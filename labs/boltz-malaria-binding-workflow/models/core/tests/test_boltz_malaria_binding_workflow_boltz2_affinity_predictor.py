...
def test_generated_structure_paths_remain_absolute_without_canonicalizing(biosim, tmp_path):
    ...
    assert module.visualize() is None
    assert module._output_payloads["structure_artifacts"]["prediction_dir_name"] == "request"
    assert "prediction_dir" not in module._output_payloads["structure_artifacts"]
    assert "/../" in module._output_payloads["structure_artifacts"]["structure_file"]
