...
def test_generated_structure_paths_remain_absolute_without_canonicalizing(biosim, tmp_path):
    ...
    module._cached_payloads = {
        ...
        "structure_artifacts": {
            "prediction_dir_name": prediction_dir.name,
    ...
    assert module.visualize() is None
    assert module._cached_payloads["structure_artifacts"]["prediction_dir_name"] == "request"
    assert "prediction_dir" not in module._cached_payloads["structure_artifacts"]
    assert "/../" in module._cached_payloads["structure_artifacts"]["structure_file"]
...
def test_advance_emits_progress_events_for_long_steps(biosim, tmp_path, monkeypatch, capsys):
    ...
    def fake_run(command, cwd, capture_output, text, timeout, check):  # noqa: ARG001
        ...
        (prediction_dir / "request_model_0.cif").write_text("data_mock\n", encoding="utf-8")
        (prediction_dir / "confidence_request_model_0.json").write_text(
            json.dumps({"confidence_score": 0.91, "ptm": 0.82}),
            encoding="utf-8",
        )
        (prediction_dir / "affinity_request.json").write_text(
            json.dumps({"affinity_probability_binary": 0.97}),
            encoding="utf-8",
        )
        return subprocess.CompletedProcess(command, 0, stdout="ok", stderr="")
...
def test_example_files_parse_and_reference_real_interface(biosim):
    ...
    assert minimal["model"]["path"].endswith("/models/core")
    assert wiring["models"][0]["path"].endswith("/models/core")
